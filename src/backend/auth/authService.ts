...
export class AuthService {
    ...
    async login(username: string, password: string): Promise<AuthResult | undefined> {

        const rows = await this.database.query<AuthUser>('SELECT * FROM users WHERE username=$1 and password=$2',
            [username, password]);
